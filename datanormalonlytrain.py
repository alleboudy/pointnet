...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data,current_rgb, current_label = provider.loadOnlyNormalsDataFile(TRAIN_FILES[train_file_idxs[fn]])
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data,current_rgb, current_label = provider.loadOnlyNormalsDataFile(TEST_FILES[fn])
        current_data = current_data[:,0:NUM_POINT,:]
        current_rgb = current_rgb[:,0:NUM_POINT,:]
        current_label = np.squeeze(current_label)
