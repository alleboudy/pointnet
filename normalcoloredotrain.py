...
parser.add_argument('--model', default='pointnet_coloredNormals', help='Model name: pointnet_cls or pointnet_cls_basic [default: pointnet_cls]')
parser.add_argument('--log_dir', default='logNORMALSncolored', help='Log dir [default: log]')
...
def train():
    with tf.Graph().as_default():
        ...
        ops = {'pointclouds_pl': pointclouds_pl,
                'pointclouds_rgb_pl':pointclouds_rgb_pl,
               'labels_pl': labels_pl,
    ...
    for fn in range(len(TRAIN_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            feed_dict = {ops['pointclouds_pl']: jittered_data,
                         ...
                         ops['pointclouds_rgb_pl']: current_rgb[start_idx:end_idx],
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data,current_rgb, current_label = provider.loadColoredNormalsDataFile(TEST_FILES[fn])
        current_data = current_data[:,0:NUM_POINT,:]
        current_rgb = current_rgb[:,0:NUM_POINT,:]
        current_label = np.squeeze(current_label)
