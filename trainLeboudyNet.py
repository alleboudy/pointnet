...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        log_string('----' + str(fn) + '-----')
        current_data, posesx,posesq = provider.loadPosesDataFile(TRAIN_FILES[train_file_idxs[fn]])
        current_data = current_data[:,0:NUM_POINT,:]
        current_data, posesx,posesq, _ = provider.shuffle_poses_data(current_data,posesx,posesq)            
        ...
        for batch_idx in range(num_batches):
            ...
            feed_dict = {ops['pointclouds_pl']: jittered_data,
                         ops['posesx']: posesx[start_idx:end_idx,:],
                         ops['posesq']: posesq[start_idx:end_idx,:],
                         ops['is_training_pl']: is_training,}
            summary, step, _, loss_val,predictedposesx,predictedposesq = sess.run([ops['merged'], ops['step'],
                ops['train_op'], ops['loss'],ops['predictedposesx'],ops['predictedposesq']],
                 feed_dict=feed_dict)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            feed_dict = {ops['pointclouds_pl']: current_data[start_idx:end_idx, :, :],
                         ...
                         ops['posesq']: posesq[start_idx:end_idx,:],
                         ops['is_training_pl']: is_training}
            summary, step, loss_val ,predictedposesx,predictedposesq = sess.run([ops['merged'], ops['step'],
                ops['loss'], ops['predictedposesx'],ops['predictedposesq']], feed_dict=feed_dict)
            #pred_val = np.argmax(pred_val, 1)
            #correct = np.sum(pred_val == current_label[start_idx:end_idx])
            #total_correct += correct
            total_seen += BATCH_SIZE
    ...
    #accuracylogstr =str (total_correct / float(total_seen))
    with open(os.path.join(LOG_DIR, 'evallog.txt'), 'a') as myfile:
         myfile.write(meanlosslogstr+'\n')
